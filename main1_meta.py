...
class CoSeLearner_Trainer(object):
    ...
    def train(self):
        ...
        for iteration, batch in tqdm(enumerate(self.data_loader['train']()), desc=f"Training"):
            ...
            ############################################################################################################
            query_acc_generations, total_loss, query_cls_loss_generations = \
                                                        self.compute_train_loss_pred(all_label_in_clsm,       # torch.Size([16, 10, 10]) for 1-shot  
                                                                                        L_cls_similarity,     # torch.Size([16, 10, 10]) for 1-shot
                                                                                        A_cls_similarity,     # torch.Size([16, 10, 10]) for 1-shot
                                                                                        B_cls_similarity,     # torch.Size([16, 10, 10]) for 1-shot                                                                                               
                                                                                        L_emb_similarity,     # torch.Size([16, 10, 10]) for 1-shot
                                                                                        query_cls_mask,       # torch.Size([16, 10, 10]) for 1-shot
            ...
            if self.global_step % self.arg.log_step == 0:
                self.log.info('step : {}  train_cls_loss : {}  query_acc_last_gen : {}'.format(self.global_step,
                                                                                        query_cls_loss_generations[-1],
                                                                                        query_acc_generations[-1]))
            ...
            if self.global_step % self.eval_opt['interval'] == 0:
                ...
                }, is_best, os.path.join('result', self.config['dataset_name'], 
                                         self.arg.checkpoint_dir, self.arg.exp_name+str('_{}gen'.format(self.config['num_generation']))))
    ...
    def compute_train_loss_pred(self, all_label_in_clsm,  
        ...
        ##########################
        total_loss_gens = [
            query_clsm_loss_generation + color_loss_coeff * query_emb_pred_loss_
        ...
        if usage:
            ...
            for l in range(num_loss - 1): total_loss += [total_loss_gens[l].view(-1) * self.config['generation_weight']]
            total_loss += [total_loss_gens[-1].view(-1) * 1.0]
        else:
            num_loss = self.config['num_generation']
            for l in range(num_loss): total_loss += [total_loss_gens[l].view(-1)]
...
def main():
    ...
    parser.add_argument('--log_dir', type=str, default=os.path.join('.', 'logs_meta'), 
