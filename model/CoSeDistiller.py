...
class SimMetric(nn.Module):
    ...
    def forward(self, embedding_gen, cls_matrix, distance_metric):  
        ###########################################
        # 1.Calculate the similarity of embeddings.
        ###########################################
        emb_i = embedding_gen.unsqueeze(2)              # emb_i: torch.Size([60, 10, 1, 128])
        emb_j = torch.transpose(emb_i, 1, 2)            # emb_j: torch.Size([60, 1, 10, 128])
        if distance_metric == 'l2':   emb_similarity = (emb_i - emb_j)**2        # l2 emb_similarity: torch.Size([60, 10, 10, 128])
        elif distance_metric == 'l1': emb_similarity = torch.abs(emb_i - emb_j)  # l1 emb_similarity: torch.Size([60, 10, 10, 128])
        embed_sim = -torch.sum(emb_similarity, 3)                                # embed_sim: torch.Size([60, 10, 10])
...
class L_embed_update(nn.Module):
    ...
    def forward(self, cls_matrix, L_embedding):
        """
        :param cls_matrix: current generation's edge feature of Color graph, Ed_(l)
        :param L_embedding: last generation's node feature of color graph, Vp_(l-1)
        :return: current generation's node feature of color graph, Vp_(l)
        """
        # get size
        ...
        ##########################################
        # 1.[cls_matirx, L_embedding]->L_embedding
        ##########################################
...
##################################
class CoSeLearner_stu(nn.Module):
    def __init__(self, encoder_flag, emb_size, num_generations, dropout, num_support_sample, num_sample, loss_indicator, dis_metric,
        ...
        :param dis_metric: metric for distance
        """
        super(CoSeLearner_stu, self).__init__()
...
def CoSeLearner_stu_(encoder_flag=False, **kwargs):
    """constructs CoSeLearner_stu network."""
    print(Fore.RED+'*********'* 10)
    print(Fore.BLUE+'<<CoSeLearner Architecture>>:')
    model = CoSeLearner_stu(encoder_flag=encoder_flag, emb_size=128, num_generations=2, dropout=0.1, loss_indicator= [1, 1, 1, 0], 
...
if __name__ == '__main__':
    ...
    model = CoSeLearner_stu(encoder_flag = encoder_flag).cuda() 
    ...
    print('L_clsm:', L_clsm[0].shape)                         # CoSeDistiller: torch.Size([16, 10, 10])
