...
class CoSeDistiller_Trainer(object):
    def __init__(self, student_net, data_loader, log, arg, config, best_step):
        """
...
def main():
    ...
    parser.add_argument('--log_dir', type=str, default=os.path.join('.', 'logs_distill_cross_domain'), 
    ...
    parser.add_argument('--color_factor', type=int, default=1, help='coefficient of color loss in total loss')
    parser.add_argument('--distill_lr', type=int, default=0.01, help='coefficient of color loss in total loss')
    ...
    ######################################################################################################################################
    # 4.Construct CoSeDistiller and execute network training and validation
    ######################################################################################################################################
    trainer = CoSeDistiller_Trainer(student_net=cose_student, data_loader=data_loader, \
